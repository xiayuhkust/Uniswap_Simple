...
contract CreatePools is Script {
    ...
    function run() public {
        ...
        address pool1 = UniswapV3Factory(FACTORY).createPool(WTURA, TT1, 3000);
        console.log("WTURA/TT1 pool created at:", pool1);

        // Create WTURA/TT2 pool (0.3% fee)
        address pool2 = UniswapV3Factory(FACTORY).createPool(WTURA, TT2, 3000);
        console.log("WTURA/TT2 pool created at:", pool2);

        // Create TT1/TT2 pool (0.05% fee)
        address pool3 = UniswapV3Factory(FACTORY).createPool(TT1, TT2, 500);
